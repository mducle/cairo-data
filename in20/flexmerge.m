function [x,y,mn,setindex] = flexmerge(runnums,datafolder,norm_mn,temperature_threshold)
...
% ---------------------------------------------------------------------------------------------------------------------------------- %
monitor = 'M1';                   % The name of the primary monitor to normalise to (either M1 or M2, etc.)
mon2 = 'M2';                      % The name of the secondary monitor to use to screen spurions
...
step_threshold = 60;              % The percent of the step size below which data points will be considered to be repeated. Eg. if =11
...
m2max_threshold = 40/3e5;         % Threshold of (mon2/mon1) above which to treat point as a spurion and ignore
...
for irun=1:length(runnums)
   filename=[datafolder dirsep sprintf('%06.f',runnums(irun))];
   if(~exist(filename,'file')); nothkle(irun) = 1; continue; end
   [data,datastr,pres,pscan,stepvec,stepstr,com,header,head]=illdata(filename);
   mydatastr = cellstr(datastr);
   [tmpvar,mColInd] = intersect(mydatastr,monitor);
   % Checks that this is an HKLE scan and if there is any data in it
   if isempty(findstr(com,'QH')) || sum(data(:,mColInd))<10
      nothkle(irun) = 1; 
      continue;   % Skips the rest of the loop...
   end
   % Determines whether ki or kf is fixed
   ifx = strfind(header,'FX='); fx=str2num(header((ifx+3):(ifx+4)));
   if(fx==1) 
   ...
   for iii = 1:length(istepstr)
      ...
      for iiii = 1:length(iequals)
         ...
         mystepstr{ii} = stepline((iequals(iiii)-8):(iequals(iiii)-7));
         ii=ii+1; 
      end
   end
   stepvec = mystepvec;
   ...
   [tmpvar,m2ClInd] = intersect(mydatastr,mon2);
   m2vl{irun} = data(tColInd,m2ClInd);
...
for iset = 1:length(s_scan_numbers)
   ...
   for irun = 1:length(s_scan_numbers{iset})
      ...
      for ix = 1:length(xval{s_indices{iset}(irun)})                     % Loop through each value of x (e.g. EN)
         ...
         if((m2vl{s_indices{iset}(irun)}(ix)/mval{s_indices{iset}(irun)}(ix))>m2max_threshold)
            continue;
         end
...
if nargin>2
   ...
   x = s_x; y = s_y; mn = s_e;  m2 = s_m2;
else
   x = s_x; y = s_y; mn = s_mn; m2 = s_m2;
end
...
if nargout<1
   for iset = 1:length(s_scan_numbers)
      ...
      disp(['| Scans:  ' scanstr]);
      disp('|------------------------------------------------------------|');
     %disp([s_independent_var{iset} sprintf('\t') countstr sprintf('\t') mnstr sprintf('\t\tM2/MN*1e5')]);
