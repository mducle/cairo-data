...
elseif strcmp(cmd,'browse')
	[file path]=uigetfile('*.m','MFIT: Choose file');	% Uigetfile to choose .m file
	if file~=0
...
%----- Read name,file,dir from edit boxes ----------------------------
	...
	dir =get(findobj('tag','mf_brows_dir'),'string');		% Get directory

	if exist([dir file])~=2											% Check it exists
		errordlg([dir file ' doesn''t exist','MFIT error'])
		return
	end
...
%----- Do the updating -----------------------------------------------------
	...
	set(hdir, 'userdata',str2mat(get(hdir, 'userdata'),dir));
	
	% Callback string for menu
	Call=[['set(findobj(''tag'',''' deblank(NameTag(i,:)) '''),''string'',''' name '''); ']...
         ['set(findobj(''tag'',''' deblank(DirTag(i,:))  '''),''string'',''' dir '''); ']...
