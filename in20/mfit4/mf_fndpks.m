function [peak,pmax,sy,ny,sdy,ndy,sddy,nddy,pmin] = mf_fndpks(x,ys,thr, pos,width,pmin,pmax,npoints)
%Quick search of peaks in a signal.
%Syntax: [peakmat,pmax,sy,ny,sdy,ndy,sddy,nddy,pmin] = mf_fndpks(x,y,{threshold=1, pos,width,,pmin,pmax,npoints}) or mf_fndpks(y)
%
...
%	npoints is point number used for smoothing. Order is 2.
...
if ~exist('pos') | ~exist('width') | isempty(pos) | isempty(width)
	nd = length(x);
	pos = x(ceil(nd/2));
	width = abs(x(nd)-x(1));
end
