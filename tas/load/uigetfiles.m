function [filename, pathname] = uigetfiles(filterspec, title, x, y)
...
% calls:   javax.swing.JFrame, MultiFileChooser, SimpleFilter
...
if isstruct(filterspec) % callback use
  ...
  case 'ok'
    ...
    p = UD.Path; index = strmatch(p, previous, 'exact');
    if isempty(index), 
      previous = { p, previous{:} };
...
if ~ischar(filterspec) & ~iscellstr(filterspec)
  disp('iFiles/uigetfiles: "filterspec" should be a char or cell string array.')
  return
end
...
previous = get(UD.Handle.Previous, 'UserData');
p = [ matlabroot filesep ]; index = strmatch(p, previous, 'exact');
if isempty(index), previous = { p, previous{:} }; end
p = [getenv('HOME') filesep ]; index = strmatch(p, previous, 'exact');
if isempty(index), previous = { p, previous{:} }; end
p = [pwd filesep ]; index = strmatch(p, previous, 'exact');
if isempty(index), previous = { p, previous{:} }; end
...
if length(fig)  % yes: exists -> raise
  ...
  h = uicontrol('Style','pushbutton', 'Tag','UIGetFiles.OK',...
                ...
                'ForegroundColor',[ 0 0.5 0 ],'ToolTipString',...
                  'Click here to accept selection', ...
...
if ~isempty(UD.Dir_orig)
  ...
  dir_date = datenum({dir_list.date}); 
  ...
  % catenate list items
  tmp3 = cellstr(strcat(' [', num2str(dir_bytes'), ' bytes]'));
  ...
  sort_list  = get(UD.Handle.Sort , 'String');
  if sort_index>0 & sort_index<=length(sort_list) & iscellstr(sort_list)
