...
if ~exist(datadir), datadir = pwd; end
...
if nargin==0 & isempty(findstr(loadfun,'frombase'))
  ...
  dirsav = pwd;
  if ~isempty(datadir)
    if exist(datadir,'dir'), cd(datadir); end
  end
  [datafile, datadir]=uigetfile(mask,'Select data file');
  cd(dirsav);
...
%------------ Call load function --------------------
	...
	[x y err xlab ylab]=feval(loadfun, [datadir filesep datafile]);
	titl = [];

	filedata = dir([ datadir filesep datafile]);
	if isempty(filedata), filedata = 0;  filedata.bytes=0; filedata.date=datestr(now); end
	set(findobj('Tag','mf_DataFile'),'ToolTipString', ...
	  [ datadir filesep datafile sprintf('\n') ...
